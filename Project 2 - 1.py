...
class HumanPlayer(Player):
    ...
    def move(self):
        ...
        while throw != 'rock'and throw != 'paper'and throw != 'scissors':
            print('spelling mistake, sorry try again')
            throw = input('rock, paper, scissors? >')
        return (throw)
...
class Game():
    ...
    def play(self, move1, move2):
            print(f"You played {self.p1.score}")
            print(f"Opponent played {move2}")
            if beats(move1, move2):
                print ("** PLAYER ONE WINS **")
                print(f"Score: Player 1: {self.p1.score}  Player 2: {move2}\n\n")
                self.p1.score += 1
                return 1
...
def beats(one, two):
    return ((one == 'rock' and two == 'scissors') and
            (one == 'scissors' and two == 'paper') and
...
if __name__ == '__main__':
    ...
    p2 = input('Select the RPS game you would like to play or just press any\
 key and enter for random game: [1]Rock, [2]Random,\
[3]ReflectPlayer, or [4]CyclePlayer: >')


    while p2 != 1 and p2 != 2 and p2 != 3 and p2 != 4:
